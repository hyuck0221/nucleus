...
DMG를 열고 `Nucleus.app`을 `Applications`로 드래그한 뒤 실행하면 됩니다. 앱은 기본값인 `0.0.0.0:8787`로 로컬 서버를 시작하고, 네이티브 macOS WebView 창 안에서 대시보드를 보여줍니다. 앱 로그는 `~/Library/Logs/Nucleus/nucleus.log`에 기록됩니다.
